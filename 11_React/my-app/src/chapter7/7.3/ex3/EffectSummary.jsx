...
function EffectSummary() {
  ...
  // 1) 렌더링 직후에 실행됨
  // 2) 마운트 시에는 무조건 실행됨
  // 3) return하는 함수는 언마운트 시에 무조건 실행됨
 ...
 // 마운트 될 때만 실행됨
  useEffect(() => {
    console.log('%cname이 변함', 'color: #03c9a8; background:#befce2;');
   ...
   // 마운트 될 때만 실행됨
   useEffect(() => {
    console.log('%c마운트 될 때만 실행', 'color: green; background:#aaffbf;');

    return ( ) => {
      // 언마운트 될 때만 실행
      console.log('%c언마운트 될 때만 실행', 'color: #0c0124; background:#e288eb;');
    }
