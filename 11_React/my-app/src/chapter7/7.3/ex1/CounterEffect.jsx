...
function CounterEffect() {
  ...
  // 실행시점 : HTML DOM 렌더링 직후(즉, 우리가 만든 컴포넌트가 화면에 나타난 상태 이후)
  useEffect(() => {
    console.log(`effect 실행!`);

    // DOM API를 사용하여 문서 타이틀 업데이트
    // document.title = `clicked ${count} times`;
  },[]); 
  // },[]); 의존성 배열을 빈 배열로 넣으면 마운트 될때만 실행
  // });  의존성 배열을 넣지 않으면 렌더링 될 때마다 실행됨
  ...
  return (
    <>
      ...
      <button type="button" onClick={() => {
        ...
        // 1. 초기에 타이틀이 바뀌지 않고 있다.(왜져? -> 처음 마운트 시 실행 안됨)
        // 2. 버튼 클릭 시 count 값이 제대로 반영 안됨(왜져? -> set함수는 비동기 함수)
